...
<div class="container pe-0">
    ...
    <ul class="nav nav-tabs mb-3" id="myTab" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <button class="nav-link {{ request('tab') == 'xuathang' ? 'active' : '' }}" id="xuathang-tab" data-bs-toggle="tab" data-bs-target="#xuathang" type="button" role="tab">
                Thống kê xuất hàng
            </button>
        </li>
        <li class="nav-item" role="presentation">
    ...
    <div class="tab-content" id="myTabContent">
        <!-- hàng hóa -->
        <div class="tab-pane fade {{ request('tab', 'ton') == 'ton' ? 'show active' : '' }}" id="ton" role="tabpanel">
        ...
        <!-- xuất hàng -->
        <div class="tab-pane fade {{ request('tab') == 'xuathang' ? 'show active' : '' }}" id="xuathang" role="tabpanel">
            <form method="GET" action="{{ route('thongke') }}" class="d-flex gap-3 align-items-center mb-3">
            ...
            <div>
                <p><strong>Tổng số phiếu xuất:</strong> {{ $tongPhieuXuat ?? 'Không có dữ liệu' }}</p>
                <p><strong>Tổng số lượng hàng xuất:</strong> {{ $tongSoLuongXuat ?? 'Không có dữ liệu' }}</p>
            </div>
        </div>

        <!-- biểu đồ -->
        <div class="tab-pane fade {{ request('tab') == 'nhapxuat' ? 'show active' : '' }}" id="nhapxuat" role="tabpanel">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (tab === 'nhapxuat') {
            ...
            new bootstrap.Tab(document.querySelector('#xuathang-tab')).show();
        } else {
            new bootstrap.Tab(document.querySelector('#ton-tab')).show(); // mặc định
        }
