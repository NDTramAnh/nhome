        ...
        <div class="card-body">
          ...
          <form action="{{ route('products.update', $product->id) }}" method="POST">
            ...
            <div class="mb-3">
              ...
              <input type="number" class="form-control" step="0.01" id="price" name="price" value="{{ old('price', $product->price) }}" required min="0">
            </div>

            <div class="mb-4">
              <label for="status" class="form-label">Tình trạng</label>
              <select class="form-select" id="status" name="status" required>
                <option value="" disabled {{ old('status', $product->status) === null ? 'selected' : '' }}>-- Chọn tình trạng --</option>
